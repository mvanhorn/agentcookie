...
- Local processes while an agent-browser attach endpoint is open. `agentcookie attach` (v0.14) drives an agent browser against a Chrome DevTools Protocol endpoint -- either your real Chrome (enabled at `chrome://inspect#remote-debugging` on Chrome 144+) or a dedicated loopback debug Chrome (`--fallback`). agentcookie binds the debug port to `127.0.0.1` only and prefers the user-gated `chrome://inspect` path (per-session approval) over a standing open port, but while that endpoint is live, **any process running as the same user can connect to it** and read/drive that Chrome -- which is exactly why Chrome 136 stopped honoring `--remote-debugging-port` on the default profile. Treat an open attach endpoint as a same-user trust boundary: enable it when you are running agents, and disable the `chrome://inspect` toggle (or close the `--fallback` debug Chrome) when you are not. The fallback's separate profile gets its own Chrome encryption key, so its debug port never exposes the real profile's jar; device-bound (DBSC) sessions never transfer to it.
